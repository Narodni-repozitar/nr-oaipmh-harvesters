...
class NUSLTransformer(OAIRuleTransformer):
    def __init__(self, allowed_slugs=None, **kwargs):
        super().__init__(**kwargs)
        if isinstance(allowed_slugs, str):
            slugs = {s.strip() for s in allowed_slugs.split(",") if s.strip()}
            self._allowed_slugs = slugs or None
        elif allowed_slugs:
...
NUSL_ID_TO_SLUG_MAPPING = {
    ...
    "archip": "52fb",
    "archiv_doc_jiri_soucek": "generic",
    "archiv_ing_arch_jana_moucky": "generic",
    "arnika": "y0dw",
